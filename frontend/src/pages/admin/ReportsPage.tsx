mport { useData } from "@/context/DataContext";
...
import { Download, FileText, Filter, PieChart as PieChartIcon, BarChart as BarChartIcon } from "lucide-react";
import { format } from "date-fns";

export default function ReportsPage() {
  const { members, payments, loans, reports, isLoading } = useData();
  const [reportType, setReportType] = useState<"financial" | "member" | "loan">("financial");
  ...
  const paymentChartData = Object.entries(paymentsByType).map(([type, amount]) => ({
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{members.filter(m => m.duesOwing === 0).length}</div>
              </CardContent>
            </Card>
            
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Members with Outstanding Dues</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{members.filter(m => m.duesOwing > 0).length}</div>
              </CardContent>
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{loans.filter(l => l.status === 'rejected').length}</div>
              </CardContent>
            </Card>
          </div>
                  ...
                  <PieChart>
                    <Pie
                      data={
                        ...
                        ).map(([purpose, amount]) => ({ name: purpose, value: amount }))
                      }
                      cx="50%"
                      cy="50%"
                      labelLine={false}
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {format(new Date(report.dateRange.from), "MMM d, yyyy")} - 
                            {format(new Date(report.dateRange.to), "MMM d, yyyy")}
                          </p>
                        </div>
                      </div>
